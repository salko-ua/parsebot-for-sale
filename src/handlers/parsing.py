...
async def add_template(query: CallbackQuery, state: FSMContext):
    ...
    if template is None or template == "":
        await query.answer(
            "Ви ще не маєте шаблону додайте його в налаштуваннях", show_alert=True
        )
        return
...
async def delete_template(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.edit_reply_markup(
        reply_markup=edit_parse_advert(template=False)
    )
...
async def repost_to_group(query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await query.answer(
            f"Не можу знайти Групу/Канал в яку надсилати оголошення", show_alert=True
        )
